...
(function($){$(function(){$(document).on('rtcl_recaptcha_loaded',function(){const $agentContactForms=$("form.agent-email-form, form#agent-email-form");if($agentContactForms.length&&typeof grecaptcha!=='undefined'&&$.inArray("agent_contact",rtcl.recaptcha.on)!==-1){$agentContactForms.each((index,form)=>{const $form=$(form);if(!$form.data('reCaptchaId')){const args={'sitekey':rtcl.recaptcha.site_key};if($form.find('#rtcl-agent-contact-g-recaptcha').length){$form.data('reCaptchaId',grecaptcha.render($form.find('#rtcl-agent-contact-g-recaptcha')[0],args))}else if($form.find('.rtcl-g-recaptcha-wrap').length){$form.data('reCaptchaId',grecaptcha.render($form.find('.rtcl-g-recaptcha-wrap')[0],args))}}})}});if($.fn.validate){$("form.agent-email-form, form#agent-email-form").each(function(){$(this).validate({submitHandler:function(form){var $form=$(form),targetBtn=$form.find('.sc-submit'),responseHolder=$form.find('.rtcl-response'),msgHolder=$("<div class='alert'></div>"),recaptchId=$form.data('reCaptchaId'),sc_response='';if(rtcl.recaptcha&&typeof grecaptcha!=='undefined'&&rtcl.recaptcha.on&&$.inArray("agent_contact",rtcl.recaptcha.on)!==-1){if(rtcl.recaptcha.v===2&&recaptchId!==undefined){const response=grecaptcha.getResponse(recaptchId);responseHolder.html('');if(0===response.length){responseHolder.removeClass('text-success').addClass('text-danger').html(rtcl.recaptcha.msg.invalid);grecaptcha.reset(recaptchId);return!1}
submit_agent_form_data_ajax(response);return!1}else if(rtcl.recaptcha.v===3){grecaptcha.ready(function(){$form.rtclBlock();grecaptcha.execute(rtcl.recaptcha.site_key,{action:'agent_contact'}).then(function(token){$form.rtclUnblock();submit_agent_form_data_ajax(token)})});return!1}}
submit_agent_form_data_ajax();return!1;function submit_agent_form_data_ajax(recaptcha_token){var formData=new FormData(form);formData.append('action','rtcl_send_mail_to_agent');formData.append('user_id',rtcl_agent.user_id||0);formData.append('__rtcl_wpnonce',rtcl.__rtcl_wpnonce);if(recaptcha_token){formData.append('g-recaptcha-response',recaptcha_token)}
$.ajax({url:rtcl_agent.ajaxurl,dataType:'json',data:formData,type:'POST',processData:!1,contentType:!1,cache:!1,beforeSend:function(){$form.rtclBlock();$form.addClass("rtcl-loading");$form.find('input textarea').prop("disabled",!0);targetBtn.prop("disabled",!0);responseHolder.html('');$('<span class="rtcl-icon-spinner animate-spin"></span>').insertAfter(targetBtn)},success:function(response){$form.rtclUnblock();targetBtn.prop("disabled",!1).next('.rtcl-icon-spinner').remove();$form.find('input textarea').prop("disabled",!1);$form.removeClass("rtcl-loading");if(response.success){msgHolder.removeClass('alert-danger').addClass('alert-success').html(response.data.message).appendTo(responseHolder);$form[0].reset();if($form.parent("#agent-email-area").parent().data('hide')!==0){setTimeout(function(){responseHolder.html('')},1000)}}else{msgHolder.removeClass('alert-success').addClass('alert-danger').html(response.data.error).appendTo(responseHolder)}
...
catch(e){console.error("WP-Optimize Minify: An error has occurred in the minified code. \n\n- Original script: https://www.radiustheme.com/demo/wordpress/themes/homlisti/wp-content/plugins/rtcl-agent/assets/js/agent.js\n- Error message: "+ e.message);}
